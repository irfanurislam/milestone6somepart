const data = [
  ...
  const carscontain = document.getElementById('car-contain');
  ...
  data1.forEach((cars) => {
   ...
   <img src="${cars.imageURL}" class="card-img-top" alt="...">
   <div class="card-body">
     <h5 class="card-title">Car Name : ${cars.name}</h5>
     <p class="card-text">${cars.description}</p>
     <a href="#" class="btn btn-primary"> car price:${cars.price}</a>
   </div>
   </div>
   
   `
  carscontain.appendChild(div);
  })
